...
export const userExists = async (playerId:string)=>{
    try {
...
export const createGame = async (gameId:string,playerId:string) => {
...
export const updateGame = async (gameId:string,playerId:string) => {
...
export const updateGameStatus = async (gameId:string,status:string,color:string) => {
    // check if game not exist
    const game = await prisma.game.findFirst({
        where:{gameId:gameId}
    })
...
export const gameDraw = async (gameId:string,status:string) => {
    // check if game not exist
    const game = await prisma.game.findFirst({
        where:{gameId:gameId}
    })
...
export const playerResume = async (gameId:string,newPlayerId:string,oldPlayerId:string) => {
    const game = await gameExists((gameId));
    if(!game){
        throw new Error("No game exists!")
    }
    if(game.blackPlayerId !== oldPlayerId && game.whitePlayerId !== oldPlayerId ) throw new Error("Player doesn't belong to the game!");
